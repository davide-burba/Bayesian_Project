#Modello con variabili random aventi prior DP. Covariate suggerite da S&S.
...
data <- list(y=RNFL_average, npat=npat, numerosity = numerosity, kk=kk, 
             m1=0, Kmax=Kmax, alpha = 0.5,  tau2 = 0.01,  X=X, Z=Z, 
             mu0=mu0, S0=S0)      # dati che passo a jags
...
modelRegress=jags.model("LMM_DP_2.bug",data=data,inits=inits,n.adapt=1000,n.chains=1)
...
save.image("../R_object/model_17.RData")

load("../R_object/model_17.RData")
...
for (i in 1:npat){
  ...
  for(j in 1:numerosity[i])
  {
    pred[kk[i]+j]=mean(tmp[kk[i]+j,] + Z[kk[i]+j,]%*%t(tmp_b))
  }
}
...
Indices=list(res.bayes=MEAN.RES.BAYES,LPML=LPML,AIC=AIC,BIC=BIC)
Indices
save(Indices,file="../R_object/model_17_indices.RData")

#Residui bayesiani aumentano leggermente rispetto al modello precedente con meno covariate (strano);
